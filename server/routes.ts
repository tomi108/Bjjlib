...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/tags/:id/rename", async (req, res) => {
    try {
      const sessionId = req.cookies.adminSessionId;
      ...
      const tag = await storage.renameTag(id, newName);
      if (!tag) {
  ...
  app.patch("/api/tags/:id/category", async (req, res) => {
    try {
      const sessionId = req.cookies.adminSessionId;
      ...
      if (categoryId !== null && (isNaN(parseInt(categoryId)) || parseInt(categoryId) < 1)) {
        return res.status(400).json({ message: "Invalid category ID" });
      }

      const tag = await storage.moveTagToCategory(id, categoryId === null ? null : parseInt(categoryId));
      if (!tag) {
        return res.status(404).json({ message: "Tag not found" });
      }
      ...
      res.status(500).json({ message: "Failed to move tag" });
    }
  });
  ...
  app.post("/api/tag-categories", async (req, res) => {
    try {
      ...
      const category = await storage.createTagCategory({
        name: name.trim(),
  ...
  app.patch("/api/tag-categories/:id", async (req, res) => {
    try {
      const sessionId = req.cookies.adminSessionId;
      ...
      const category = await storage.updateTagCategory(id, updates);
      if (!category) {
  ...
  app.delete("/api/tag-categories/:id", async (req, res) => {
    try {
      ...
      const deleted = await storage.deleteTagCategory(id);
      if (!deleted) {
        return res.status(404).json({ message: "Category not found" });
      }
